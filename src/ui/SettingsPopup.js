...
export class SettingsPopup extends BaseUI {
  ...
  constructor(screenWidth, screenHeight) {
    ...
    this.diffLabel.position.set(panelX + 40, currentY);
    this.container.addChild(this.diffLabel);

    currentY += 100;

    // --- Difficulty Buttons ---
    this.btnEasy = new UIButton("EASY", {
      width: 90,
      height: 40,
      bgColor: 0x4caf50,
      fontSize: 16,
    ...
    this.btnNormal = new UIButton("NORMAL", {
      width: 90,
      height: 40,
      bgColor: 0x008cba,
      fontSize: 16,
    ...
    this.btnHard = new UIButton("HARD", {
      width: 90,
      height: 40,
      bgColor: 0xff4444,
      fontSize: 16,
    ...
    // --- Reset Best Score ---
    this.btnReset = new UIButton("RESET BEST SCORE", {
      width: 220,
      height: 40,
      bgColor: 0x883333,
      fontSize: 16,
    ...
    this.btnEasy.onClick(() => {
      this._playClickSfx();
      this._setDifficulty("EASY");
    });
    this.btnNormal.onClick(() => {
      this._playClickSfx();
      this._setDifficulty("NORMAL");
    });
    this.btnHard.onClick(() => {
      this._playClickSfx();
  ...
  resize(screenWidth, screenHeight) {
    ...
    this.panel.clear().roundRect(panelX, panelY, panelW, panelH, 16).fill({ color: 0x1a1a1a, alpha: 0.95 });
    this.title.position.set(screenWidth / 2, panelY + 45);
  ...
  onDifficultySelect(callback) {
    this._onDifficultyCb = callback;
  }
  onResetScore(callback) {
    this._onResetScoreCb = callback;
  }
}
